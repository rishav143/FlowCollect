...
function StatusChip({ lifecycle, time }: { lifecycle: LifeCycleStatus; time: TimeStatus }) {
  ...
  return (
    <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${
      ...
      dueToday ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400' :
      LIFECYCLE_STYLE[lifecycle]
    }`}>
      {overdue ? 'Overdue' : dueToday ? 'Due Today' : lifecycle.replace('_', ' ')}
    </span>
  )
}
...
function InfoRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex justify-between items-start py-2 border-b border-[#F4F7F9] dark:border-white/10 last:border-0">
      <span className="text-sm text-[#8A9BAE]">{label}</span>
...
function TabBar({ active, onChange }: { active: Tab; onChange: (t: Tab) => void }) {
  ...
  return (
    <div className="flex gap-1 border-b border-[#F4F7F9] dark:border-white/10 -mb-px">
      {tabs.map((t) => (
        <button
          key={t.id}
  ...
  if (!invoice) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[40vh] gap-3">
        <p className="text-sm text-[#8A9BAE]">Invoice not found.</p>
    ...
    <>
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div className="bg-white dark:bg-[#1B2838] rounded-xl border border-[#F4F7F9] dark:border-white/10 p-5">
              {activeTab === 'details' && (
                <div className="space-y-5">
                  ...
                  {/* Line items */}
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-[#8A9BAE] mb-3">Line Items</p>
                    <div className="space-y-2">
                      {invoice.items.length === 0 ? (
                        <p className="text-sm text-[#8A9BAE]">No line items</p>
                      ) : (
                        invoice.items.map((item) => (
                          <div key={item.id} className="flex justify-between items-start gap-4 py-2 border-b border-[#F4F7F9] dark:border-white/10 last:border-0">
                            <div className="min-w-0">
                              <p className="text-sm font-medium text-[#0D1B2A] dark:text-white">{item.description}</p>
                              <p className="text-xs text-[#8A9BAE]">{item.quantity} × {fmt(item.unitPrice, currency)}</p>
                            </div>
          ...
          <div className="bg-white dark:bg-[#1B2838] rounded-xl border border-[#F4F7F9] dark:border-white/10 p-5 self-start">
            <p className="text-xs font-semibold uppercase tracking-wide text-[#8A9BAE] mb-4">Payment Summary</p>
            <div className="space-y-1">
              ...
              <div className="py-2 border-b border-[#F4F7F9] dark:border-white/10 flex justify-between">
              ...
              <div className="py-2 flex justify-between">
                <span className="text-sm text-[#8A9BAE]">Remaining</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-white dark:bg-[#1B2838] rounded-2xl shadow-xl p-6">
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={() => setShowDeleteDlg(false)} className="px-4 py-2 rounded-lg text-sm font-medium text-[#8A9BAE] hover:bg-[#F4F7F9] dark:hover:bg-[#243447] transition-colors">
                Cancel
              </button>
              <button
